...
namespace ColorWar.colorGame.GameObject {
	class GameInterface {
		...
		public void draw(SpriteBatch batch, TimeSpan time) {
			...
			batch.DrawString(font, ((time.Minutes).ToString() + " : " + (time.Seconds).ToString()), new Vector2(ColorGame.WIDTH / 2 - 40, 10), Color.Black, 0, new Vector2(0, 0), 3, SpriteEffects.None, 0);
			for (int i = 0; i < 4; i++) {
				batch.DrawString(font, player1.Resources[i].ToString(), new Vector2(40, 200 + 50 * i), Color.Black, 0, new Vector2(0, 0), 2, SpriteEffects.None, 0);
				batch.DrawString(font, player2.Resources[i].ToString(), new Vector2((ColorGame.HEIGHT - ColorGame.InterfaceHeight + ColorGame.InterfaceWidth) + 40, 200 + 50 * i), Color.Black, 0, new Vector2(0, 0), 2, SpriteEffects.None, 0);
			}
